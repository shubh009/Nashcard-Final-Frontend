...
const index = () => {
  ...
  const notify = () =>
    toast.success("Your query has been send to our customer care executive!");
  ...
  const postQuestions = async () => {
    if (querytopic && qtitle && comments != null) {
      let result = await fetch(`${process.env.REACT_APP_API_URL}/addhelp`, {
        method: "post",
        body: JSON.stringify({
          userid: userid,
          qtopic: querytopic,
            ...
            <div className="Dashboardsec box-shadow helpform">
              ...
              <div className="row mt-4 mb-2">
                <div className="col-lg-5">
                  ...
                  <select className="form-control" onChange={onServiceLevelDDL}>
                    ...
                    <option value="3" name="select">
                      {" "}
                      Payment Issue
                    </option>
                    <option value="4" name="select">
                      {" "}
                      Grading Level
                    </option>
                    <option value="5" name="select">
                      {" "}
                      Review Order
                    </option>
                    <option value="6" name="select">
                      {" "}
                      Review Process
                    </option>
                  </select>
              ...
              <div className="row mt-4 mb-4">
                <div className="col-lg-6">
                  <button
                    className="btn btn-outline-primary w-100 height-theme"
                    onClick={formclear}
                  >
                    {" "}
                    Clear Form

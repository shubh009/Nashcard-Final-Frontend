...
const index = () => {
  ...
  const handelChange = async (e) => {
    ...
    let result;
    result = await fetch(
      `${process.env.REACT_APP_API_URL}/getadminfilterorderlist/`,
      {
        method: "POST",
        body: JSON.stringify({
          orderStatus: dname,
  ...
  const getorderlist = async () => {
    let result;
    result = await fetch(
      `${process.env.REACT_APP_API_URL}/getadminorderlist/`,
      {
        method: "GET",
        headers: {
          "content-type": "application/json",
                        ...
                        {unifo.orders.map((porders, index) => {
                          return (
                            <tr key={porders._id}>
                              <td key={porders._id}>
                                {" "}
                                <span>{porders.orderid}</span>
                              </td>
                              <td>{unifo.name}</td>
                              <td>
                                {porders.localpickup === 1 ? (
                                  <label>
                                    Yes, I'll drop and pickup my cards
                                  </label>
                                ) : (
